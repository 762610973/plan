...
## 搜索
- 二分查找: 基于分治策略, 利用数据有序性, 每轮减少一般搜索范围, 直至找到目标元素或搜索区间为空为止.
- 哈希优化策略: 通过将线性查找替换为哈希查找来降低算法的时间复杂度.
  - 线性查找: 以时间换空间
  - 哈希查找: 以空间换时间
- 暴力搜索
  - 线性搜索
...
- 自适应搜索
  ...
  - 哈希查找
  - 树查找
    - 适用于海量数据, 因为树节点在内存中分散存储的
    - 适合需要维护有序数据或范围查找的场景
    - 持续增删节点的过程中, 二叉搜索树可能产生倾斜, 时间复杂度劣化是O(n)
    - 若使用AVL或红黑树, 则各项操作可在O(log n)效率下稳定运行, 但维护树平衡会增加额外开销.

## 排序算法
- 评价维度
  - 运行效率
  - 就地性: 在原数组上直接实现排序, 无需借助额外的辅助数组, 节省内存.
...
> 连续地比较与交换相邻元素实现排序.
- 每轮得到一个最大值/最小值
- 如果某轮冒泡中没有执行任何交换操作, 说明数组已经完成排序, 可直接返回结果.
- 时间复杂度O(n2)
- 稳定排序, 冒泡中遇到相等元素不交换.
### 插入排序
> 在未排序区间选择一个基准元素, 将该元素与其左侧已排序元素逐一比较大小, 并将其插入到正确位置.
- 时间复杂度O(n2)
- 空间复杂度O(1), 原地排序: 指针i和j使用常数大小的额外空间
- 稳定排序, 操作过程中, 将元素插入到相等元素的后侧, 不改变他们的顺序.
- **数据量较小的时候, 插入排序通常更快**
- 实际中, **插入排序的使用频率显著高于冒泡排序和选择排序**. 冒泡排序的计算开销通常高于插入排序.
### 快速排序
> 选择数组中某个元素作为基准数, 将所有小于基准数的元素移到其左侧, 大于基准数的元素移动到其右侧
- 时间复杂度O(nlogn)
- 空间复杂度O(n), 原地排序
- 非稳定排序
- 出现最差情况的概率很低
...
- ![](../../images/algorithm/merge_sort.png)
- 时间复杂度: O(nlogn)
- 空间复杂度: O(n), 非原地排序
- 稳定排序
### 堆排序
...
- ![](../../images/algorithm/bucket_sort.png)
- 时间复杂度: O(n+k), 桶数量k比较大时, 时间复杂度则趋向于O(n)
- 空间复杂度O(n+k)
- 桶排序是否稳定取决于排序桶内元素的算法是否稳定.
### 计数排序
> 通过统计元素数量来实现排序, 通常应用于整数数组
- ![](../../images/algorithm/count_sort.png)
- 计数排序只适用于非负整数
- 计数排序适用于数据量大但数据范围较小的情况.
- 时间复杂度: O(n+m), 一般情况下n>>m
- 空间复杂度O(n+m)
- 稳定排序
- 计数排序适用于数据量大但数据范围较小的情况
### 基数排序
> 通过统计个数来实现排序。在此基础上, 基数排序利用数字各位之间的递进关系, 依次对每一位进行排序, 从而得到最终的排序结果.
- ![](../../images/algorithm/radix_sort.png)
- 时间复杂度: O(nk)
- 空间复杂度: O(n+d), 非原地排序.
- 稳定排序: 与计数排序相同.

## 分治
> 分而治之, 通常基于递归实现, 包括分和治两个步骤
- 问题可以被分解
- 子问题是独立的
- 子问题的解可以被合并
...
## 动态规划
> 是一个重要的算法范式, 将一个问题分解为一系列更小的子问题, 并通过存储子问题的解来避免重复计算, 从而大幅提升时间效率.
## 贪心
> 其基本思想是在问题的每个决策阶段, 都选择当前看起来最优的选择, 即贪心地做出局部最优的决策, 以期望获得全局最优解
- 动态规划会根据之前阶段的所有决策来考虑当前决策, 并使用过去子问题的解来构建当前子问题的解。
- 贪心算法不会重新考虑过去的决策, 而是一路向前地进行贪心选择, 不断缩小问题范围, 直至问题被解决
- 贪心算法会迭代地做出一个又一个的贪心选择, 每轮都将问题转化为一个规模更小的子问题, 直到问题被解决.
- 硬币找零问题
- 区间调度问题
- 分数背包问题
- 股票买卖问题
